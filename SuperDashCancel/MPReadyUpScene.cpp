...
{
	bg = DrawableTexRect(Vec2(-1, -1), Vec2(2, 2), Color(0.86f, 0.85f, 0.8f, 1));
	bg.loadTexture("../SuperDashCancel/textures/texture2.png", NOALPHA);

	// setup icons
	p1Blank = DrawableTexRect(Vec2(-0.4, -0.2f),Vec2(0.09,0.16)*3, Color(0.3f, 0.3f, 0.3f,1.0f));
	p1Blank.loadTexture("../SuperDashCancel/textures/texture1.png", ALPHA);

	p2Blank = DrawableTexRect(Vec2(0.1275f, -0.2f), Vec2(0.09, 0.16) * 3, Color(0.3f, 0.3f, 0.3f, 1.0f));
	p2Blank.loadTexture("../SuperDashCancel/textures/texture1.png", ALPHA);
	p1Scale = 0.0f;
	p2Scale = 0.0f;
	p1Symbol = DrawableTexRect(Vec2(-0.4, -0.2f), Vec2(0.09, 0.16) * p1Scale, Color(0.97f, 0.94f, 0.95f, 1.0f));
	p1Symbol.loadTexture("../SuperDashCancel/textures/texture4.png", ALPHA);

	p2Symbol = DrawableTexRect(Vec2(0.1275f, -0.2f), Vec2(0.09, 0.16) * p2Scale, Color(0.6f, 0.3f, 0.3f, 1));
	p1Symbol.loadTexture("../SuperDashCancel/textures/texture4.png", ALPHA);
	//setup text
	Instruct = DrawableText(&app->fontengine, "Hold Button 1 to Ready Up", Vec2(-0.34f, -0.2f), 0.4f, Color(0.3f, 0.3f, 0.3f, 1.0f));
	p1text =  DrawableText(&app->fontengine, "Player 1", Vec2(-0.36f, 0.6f), 0.4f, Color(0.6f, 0.3f, 0.3f, 1));
	p2text = DrawableText(&app->fontengine, "Player 2", Vec2(0.16f, 0.6f), 0.4f, Color(0.8f, 0.5f, 0.65f, 1));
...
void MPReadyUpScene::Init()
{
	...
	p1Scale = 0.0f;
	p2Scale = 0.0f;
	p1Symbol.setScale(Vec2(0.27, 0.16* p1Scale));
	p2Symbol.setScale(Vec2(0.27, 0.16* p2Scale));
